...
static luacef_window_info_newindex(lua_State* L)
{
	...
	if (!strcmp(index, _wi_style))
		...
		wi->window_name = luacef_string_from_cs(lua_tostring(L, 3));
...
static int luacef_settings_new(lua_State* L)
{
	...
	if (lua_istable(L, 1)) {
		...
		if (lua_getfield(L, 1, _s_user_agent))
			s->user_agent = luacef_to_cefstring_from_cs(L, -1);

		if (lua_getfield(L, 1, _s_accept_language_list))
			s->accept_language_list = luacef_to_cefstring_from_cs(L, -1);
		...
		if (lua_getfield(L, 1, _s_browser_subprocess_path))
			s->browser_subprocess_path = luacef_to_cefstring_from_cs(L, -1);

		if (lua_getfield(L, 1, _s_cache_path)) 
			s->cache_path = luacef_to_cefstring_from_cs(L, -1);
		...
		if (lua_getfield(L, 1, _s_javascript_flags))
			s->javascript_flags = luacef_to_cefstring_from_cs(L, -1);

		if (lua_getfield(L, 1, _s_locales_dir_path))
			s->locales_dir_path = luacef_to_cefstring_from_cs(L, -1);

		if (lua_getfield(L, 1, _s_locale))
			s->locale = luacef_to_cefstring_from_cs(L, -1);

		if (lua_getfield(L, 1, _s_log_file))
			s->log_file = luacef_to_cefstring_from_cs(L, -1);
		...
		if (lua_getfield(L, 1, _s_product_version))
			s->product_version = luacef_to_cefstring_from_cs(L, -1);
		...
		if (lua_getfield(L, 1, _s_resources_dir_path))
			s->resources_dir_path = luacef_to_cefstring_from_cs(L, -1);
		...
		if (lua_getfield(L, 1, _s_user_data_path))
			s->user_data_path = luacef_to_cefstring_from_cs(L, -1);
...
static int luacef_settings_index(lua_State* L)
{
	...
	if (!strcmp(index, _s_single_process))
		...
		lua_pushstring(L, luacef_string_to_cs(s->user_agent));
...
static int luacef_settings_newindex(lua_State* L)
{
	...
	if (!strcmp(index, _s_single_process))
		...
		s->user_agent = luacef_to_cefstring_from_cs(L, 3);
	...
	*/
	...
	if (lua_istable(L, 1)) {
		...
		if (lua_getfield(L, 1, _bs_accept_language_list))
			bs->accept_language_list = luacef_to_cefstring_from_cs(L, -1); //str
		...
		if (lua_getfield(L, 1, _bs_default_encoding))
			bs->default_encoding = luacef_to_cefstring_from_cs(L, -1); //str
		...
		if (lua_getfield(L, 1, _bs_fantasy_font_family))
			bs->fantasy_font_family = luacef_to_cefstring_from_cs(L, -1); //str
		...
		if (lua_getfield(L, 1, _bs_fixed_font_family))
			bs->fixed_font_family = luacef_to_cefstring_from_cs(L, -1); //str
		...
		if (lua_getfield(L, 1, _bs_sans_serif_font_family))
			bs->sans_serif_font_family = luacef_to_cefstring_from_cs(L, -1); //str

		if (lua_getfield(L, 1, _bs_serif_font_family))
			bs->serif_font_family = luacef_to_cefstring_from_cs(L, -1); //str

		if (lua_getfield(L, 1, _bs_standard_font_family))
			bs->standard_font_family = luacef_to_cefstring_from_cs(L, -1); //str
...
int luacef_browser_settings_newindex(lua_State* L)
{
	...
	if (!strcmp(index, _bs_accept_language_list))
		bs->accept_language_list = luacef_to_cefstring_from_cs(L, 3); //str
		...
		bs->default_encoding = luacef_to_cefstring_from_cs(L, 3); //str
		...
		bs->fantasy_font_family = luacef_to_cefstring_from_cs(L, 3); //str

	else if (!strcmp(index, _bs_file_access_from_file_urls))
		bs->file_access_from_file_urls = lua_tointeger(L, 3); //state

	else if (!strcmp(index, _bs_fixed_font_family))
		bs->fixed_font_family = luacef_to_cefstring_from_cs(L, 3); //str
		...
		bs->sans_serif_font_family = luacef_to_cefstring_from_cs(L, 3); //str

	else if (!strcmp(index, _bs_serif_font_family))
		bs->serif_font_family = luacef_to_cefstring_from_cs(L, 3); //str

	else if (!strcmp(index, _bs_standard_font_family))
		bs->standard_font_family = luacef_to_cefstring_from_cs(L, 3); //str
